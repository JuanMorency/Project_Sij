...
volatile float q0 = 1.0f, q1 = 0.0f, q2 = 0.0f, q3 = 0.0f;	// quaternion of sensor frame relative to auxiliary 
...
void MadgwickAHRSupdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz) {
	...
	q0 += qDot1 * deltaTimeAk8963;
	q1 += qDot2 * deltaTimeAk8963;
	q2 += qDot3 * deltaTimeAk8963;
	q3 += qDot4 * deltaTimeAk8963;
...
void MadgwickAHRSupdateIMU(float ax, float ay, float az, float gx, float gy, float gz) {
	...
	// Integrate rate of change of quaternion to yield quaternion
	q0 += qDot1 * deltaTimeMpu9255;
	q1 += qDot2 * deltaTimeMpu9255;
	q2 += qDot3 * deltaTimeMpu9255;
	q3 += qDot4 * deltaTimeMpu9255;
