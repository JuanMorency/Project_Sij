/**
******************************************************************************
* File Name         : interrupt.cpp
* Description       : ISR for the INT pins on the uC, the timers, USART and I2C
...
/**
	* @brief Sets the registers of timer 0 for interrupt control and starts interrupts
	*/
void startInterrupt()
{
...
/**
	* @brief ISR for reading the interrupt generated by the IMU, deactivated for now
			Since the interrupt generated by the IMU is too fast for this uC
	*/
ISR(INT2_vect) {
	if(imuInitialized)
	{
...
/**
	* @brief ISR for the overflow on the counter 0 used for update rate measurement of IMU
	*/
ISR(TIMER0_OVF_vect){
...
/**
	* @brief ISR for the event counter. This sets the different flags at different frequencies 
	in order to control different hardware polling speeds.
	*/
...
/**
	* @brief ISR for reading the RX interrupt generated by the USART 
	*/
ISR(USART0_RX_vect) {
...
/**
	* @brief ISR for reading the TX interrupt generated by the USART
	*/
ISR(USART0_TX_vect) {
	if(serialInitialized)
	{
...
/**
	* @brief ISR for I2C which implements a FSM to read IMU sensors at different speeds
	*/
ISR(TWI_vect) {
